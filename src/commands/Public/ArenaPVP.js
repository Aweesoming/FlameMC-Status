...
export default class extends Command {
  ...
  async run(interaction) {
    ...
    try {
      ...
      const arenaPvPStats = playerData.realStats.pvp_arena;

      const embed = new EmbedBuilder()
        .setAuthor({ name: `Arena PVP ${nick}`, iconURL: skinURL })
        .setDescription(`\`•\` **Nick**: ${nick}.\n\`•\` **Banido**: ${playerData.banned ? 'Sim' : 'Não'}.\n# INFORMAÇÕES ⬇️\n\`•\` **Kills**: ${arenaPvPStats ? arenaPvPStats.stats.find(stat => stat.statsMap.name === 'pvp_arena_kills').value : 'N/A'}\n\`•\` **Deaths**: ${arenaPvPStats ? arenaPvPStats.stats.find(stat => stat.statsMap.name === 'pvp_arena_deaths').value : 'N/A'}\n\`•\` **Melhor Sequência**: ${arenaPvPStats ? arenaPvPStats.stats.find(stat => stat.statsMap.name === 'pvp_arena_best_streak').value : 'N/A'}`)
